...
function submit_comment() {
    $('#submitbt').click(function () {
        ...
        //     return false;
        // }
        if (!notEmpty("score", "Vui lòng chọn đánh giá của bạn về sản phẩm này!"))
            return false;
...
//cau hinh danh gia chat luong
    $('.rating-value').raty({
        starOff: 'images/sao1.svg',
        starOn: 'images/sao2.svg',
        cancel: false,
...

{
    ...
    $.ajax({
        type : 'get',
        url : '/index.php?module=products&view=product&task=update_useful&raw=1',
        dataType : 'html',
        data: 'id='+$id,
...

{
    ...
    $.ajax({
        type : 'get',
        url : '/index.php?module=products&view=product&task=update_report&raw=1',
        dataType : 'html',
        data: 'id='+$id,
        success : function(data){
            // return false;
            // $(this).addClass('active');
            Alert.render('Cảm ơn bạn đã góp ý');
            // window.location.reload();
            return true;
        },
        error : function(XMLHttpRequest, textStatus, errorThrown) {}
...
{
    var rating_cmt = $('#forma').val();
    var rating_prd = $('#forma1').val();
    // alert(rating_prd);
    // return false;
    ...
    $.ajax({
        type : 'get',
        url : '/index.php?module=comments&view=comments&raw=1&rating_cmt='+rating_cmt+'&rating_prd='+rating_prd+'&cmt_module=products&cmt_view=product',
        dataType : 'html',
        data: 'id='+$id,
        success : function(data){
            $("#_info_comment").html(data);
            // return false;
            // $(this).addClass('active');
            // Alert.render('Cảm ơn bạn đã góp ý');
            // window.location.reload();
