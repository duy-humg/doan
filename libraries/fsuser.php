...
//var_dump($password);die;
        ...
        $sql = "SELECT * FROM `" . $this->tbStore . "` WHERE published = 1 AND `telephone` = '$uname' AND `password` = $password LIMIT 1";
//        echo $sql;die;
        $db->query($sql);
        $user = $db->getObject();
        ...
        if ($loadUser) {
            ...
            $_SESSION['telephone'] = $this->userInfo->telephone;
            $_SESSION['avatar'] = $this->userInfo->telephone;
            $_SESSION['name'] = $this->userInfo->name;
//            var_dump($_SESSION['phone']);die;
