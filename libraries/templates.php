...
//	    	$global  = new FsGlobal();
        ...
        array_push($this->style, URL_ROOT . "templates/" . TEMPLATE . "/css/style.css".VERSION);
        //array_push ( $this->style, URL_ROOT . "templates/" . TEMPLATE . "/css/responsive.css" );
        array_push($this->script_top, URL_ROOT . "libraries/jquery/jquery-1.11.0.min.js".VERSION);
//        array_push($this->script_top, URL_ROOT . "libraries/jquery/jquery.ui.stars/js/jquery.min.js");
        ...
        array_push($this->script_bottom, URL_ROOT . "templates/" . TEMPLATE . "/js/masonry.pkgd.min.js".VERSION);
    ...
    function load_position($position = '', $type = '')
    {
        ...
        foreach ($block_list as $item) {
            ...
            if (method_exists('Templates', $func))
                $round = $this->$func ($title, $module_style, $item->module, $module_suffix, $id, $url, $text_replace, $text_color, $summary, $type_html);
            else
                $round [0] = $round [1] = "";
...
//        if ($Itemid != 23) {
            ...
            <head id="Head1" prefix="og: http://ogp.me/ns# fb:http://ogp.me/ns/fb# article:http://ogp.me/ns/article#">
                ...
                <meta property="fb:app_id" content="298968428552993"/>
                ...
                <meta name="google-site-verification" content="TgU2_xZ3Sp2M06bhCH8gjHsPVH_xqqFjuSyaB1l3qPE"/>
                <meta http-equiv="REFRESH" content="1800"/>
                <?php
                if (strpos('?', $_SERVER['HTTP_HOST'] . $_SERVER['REDIRECT_URL'])) {
                    ?>
                    <meta property="robots" content="unindex,dofollow">
                    <?php
                } else {
                    ?>
                ...
                if ($this->get_variables('canonical')) {
                    ?>
                    <link rel="canonical" href="<?php echo $this->get_variables('canonical'); ?>"/>
                    <?php
                } else {
                    ?>
                    <link rel="canonical"
                ...
                <link rel="alternate" media="handheld"
                      href="https://<?php echo $_SERVER['HTTP_HOST'] . $_SERVER['REDIRECT_URL']; ?>"/>
                <link rel="alternate" href="http://<?php echo $_SERVER['HTTP_HOST'] . $_SERVER['REDIRECT_URL']; ?>"
                      hreflang="x"/>
...
//                $str_top .= '<summary class="block_summary row-item"><p><span>' . $summary . '<span></p></summary>';
    ...
    function sw_get_current_weekday()
    {
        ...
        switch ($weekday) {
            case 'monday':
                $weekday = '<span>Thứ hai</span>';
                break;
            case 'tuesday':
                $weekday = '<span>Thứ ba</span>';
                break;
            case 'wednesday':
                $weekday = '<span>Thứ tư</span>';
                break;
            case 'thursday':
                $weekday = '<span>Thứ năm</span>';
                break;
            case 'friday':
                $weekday = '<span>Thứ sáu</span>';
                break;
            case 'saturday':
                $weekday = '<span>Thứ bảy</span>';
                break;
            default:
                $weekday = '<span>Chủ nhật</span>';
                break;
        }
