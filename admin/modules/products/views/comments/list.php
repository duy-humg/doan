...
//                                                var_dump($list);die;
                                                foreach ($list as $row) { ?>
        											<?php $link_detail = FSRoute::_('index.php?module=products&view=product&id='.$row->id.'&code='.$row -> alias.'&ccode='.$row-> category_alias); ?>
...
<script>
	...
	function ajax_call_comments(record_id){
		...
		$.get("index2.php?module=products&view=comments&task=ajax_get_comments_by_new&raw=1",{record_id:record_id,type:type}, function(html){
	...
	function ajax_unpublished(id,record_id){
		$.get("index2.php?module=products&view=comments&task=ajax_unpublished&raw=1",{id:id,record_id:record_id}, function(status){
			if(status == 1){
				type = $('#type_comment').val();
				$.get("index2.php?module=products&view=comments&task=ajax_get_comments_by_new&raw=1",{record_id:record_id,type:type}, function(html){
//				$.get("index2.php?module=products&view=comments&task=ajax_get_comments_by_new&raw=1",{record_id:record_id}, function(html){
					$('#comments_area').html(html);
				});
	...
	*/
	...
	function ajax_published(id,record_id){
		$.get("index2.php?module=products&view=comments&task=ajax_published&raw=1",{id:id,record_id:record_id}, function(status){
			if(status == 1){
				type = $('#type_comment').val();
				$.get("index2.php?module=products&view=comments&task=ajax_get_comments_by_new&raw=1",{record_id:record_id,type:type}, function(html){
//				$.get("index2.php?module=products&view=comments&task=ajax_get_comments_by_new&raw=1",{record_id:record_id}, function(html){
	...
	function ajax_del(id,record_id,published){
		if(confirm('Bạn chắc chắn muốn xóa?')){
			$.get("index2.php?module=products&view=comments&task=ajax_del&raw=1",{id:id,record_id:record_id}, function(status){
				if(status == 1){
					type = $('#type_comment').val();
					$.get("index2.php?module=products&view=comments&task=ajax_get_comments_by_new&raw=1",{record_id:record_id,type:type}, function(html){
						$('#comments_area').html(html);
	...
	function submit_reply(parent_id,record_id){
		...
		$.get("index.php?module=products&view=comments&task=ajax_save_comment&raw=1",{record_id:record_id,parent_id:parent_id,name: name,text:reply_content}, function(status){
			if(status == 1){
				type = $('#type_comment').val();
				$.get("index2.php?module=products&view=comments&task=ajax_get_comments_by_new&raw=1",{record_id:record_id,type:type}, function(html){
//				$.get("index2.php?module=new&view=comments&task=ajax_get_comments_by_new&raw=1",{record_id:record_id}, function(html){
	...
	function submit_edit(comment_id,record_id){
		...
		$.get("index.php?module=products&view=comments&task=ajax_edit_comment&raw=1",{record_id:record_id,comment_id:comment_id,name: name,text:edit_content}, function(status){
			if(status == 1){
				type = $('#type_comment').val();
				$.get("index2.php?module=products&view=comments&task=ajax_get_comments_by_new&raw=1",{record_id:record_id,type:type}, function(html){
					$('#comments_area').html(html);
...
//	    jQuery.ajax({
//	          type: 'POST',
//	          url: 'index2.php?module=products&view=comments&task=save_point&raw=1',
...
//	          }
//	    });
		$.get("index2.php?module=products&view=comments&task=save_point&raw=1",{comment_id:comment_id,point:point}, function(data){
			 if(data) {
	            return true;
            } else {
                alert(data);return false;
