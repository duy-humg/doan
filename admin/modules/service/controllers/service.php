<?php
	class ServiceControllersService  extends Controllers
	{
		...
		function display()
		{
			...
			$model  = $this -> model;
			$list = $model->get_data('');
            $categories = $model->get_records(' published = 1 ','fs_members_service_group','id,name',' id ASC ');
        ...
        function add()
		{
            ...
            $list_quanrity = $model->get_records(' session_id = "'.$uploadConfig.'" AND is_type = 0 ','fs_members_service_item');
            $list_quanrity2 = $model->get_records(' session_id = "'.$uploadConfig.'" AND is_type = 1 ','fs_members_service_item');
            
            $categories = $model->get_records(' published = 1 ','fs_members_service_group','id,name',' id ASC ');
            
            $categories_banner = $model->get_records(' published = 1 ','fs_banners_categories','id,name',' id ASC ');
            
            $list_post = $model->get_records(' published = 1 AND is_bonus = 1 ','fs_members_service','name,id,alias',' ordering ASC ');
		...
		function edit()
		{
            ...
            $list_quanrity = $model->get_records(' is_type = 0 AND service_id = '.$data->id,'fs_members_service_item');
            $list_quanrity2 = $model->get_records(' is_type = 1 AND service_id = '.$data->id,'fs_members_service_item');
            $categories = $model->get_records(' published = 1 ','fs_members_service_group','id,name',' id ASC ');
            
            $categories_banner = $model->get_records(' published = 1 ','fs_banners_categories','id,name',' id ASC ');
            
            $list_post = $model->get_records(' published = 1 AND is_bonus = 1 AND id != '.$id,'fs_members_service','name,id,alias',' ordering ASC ');
        ...
        function delete_item(){
            ...
            $rs = $model->_remove(' id = '.$ids,'fs_members_service_item');
            if($rs){
              echo 1;  
            }else{
              echo 0;  
