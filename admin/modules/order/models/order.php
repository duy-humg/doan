...
//        if ($id_sub && $id_sub != 0) {
        echo $query = "  SELECT a.*,b.name as product_name, b.alias as product_alias,b.category_alias,b.weight" . $select .
...
//        var_dump($id);
        global $db;
        $query = "  SELECT a.*,b.name as product_name, b.alias as product_alias,b.category_alias,b.weight 
					FROM fs_order_items AS a
					INNER JOIN fs_products AS b on a.product_id = b.id
					WHERE
						a.order_id = $id
    ...
    function get_data_order_item()
    {
        $id = FSInput::get('id', 0, 'int');
        global $db;
        $query = "  SELECT product_id_sub,id
					FROM fs_order_items
					WHERE
						order_id = $id
					";
