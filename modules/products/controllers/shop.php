...
class ProductsControllersShop extends FSControllers
{
...
//        }
        ...
        $list_bl = $model->list_bl();

        $get_shop= $model->getshop();
        $listsp_shop = $model->listsp_shop();
...
//            $tags_group = $cat -> tags_group;
        $cid = FSInput::get('cid');
        $total_pr = $model->count_total($cid);
        // var_dump($total_pr );

        $query_body = $model->set_query_body1($cid);
        $list = $model->getProductsList1($query_body);
    

       
        $total = $model->getTotal($query_body);
        $pagination = $model->getPagination_2($total_pr);
