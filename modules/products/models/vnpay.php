...
class ProductsModelsVnpay extends FSModels
{
    ...
    function getProduct($id)
    {
        $where = "";
        $where .= " AND id = " . $id;
        $query = " SELECT id,name, alias,price,price_old,price_old1,image,category_alias,discount
...
//        thong tin mua hang
        ...
        if ($code_down) {
            $dis = $this->checkCode($code_down, $after_discount_member);
        ...
        if ($money > 0) {
            ...
            $sql = "UPDATE fs_discount_code SET count=count-1 WHERE `name`='$code_down'";
...
//        var_dump($row);
    ...
    {
//        var_dump($_SESSION['info_guest']);die;
//        var_dump($_SESSION['cart']);die;

        $order = $this->get_record_by_id($id,'fs_order');
//        var_dump($order);die;
//        var_dump($order->ord_payment_type);die;
//var_dump($order);die;
        // body
        global $config;
        $body = '';
        $body .= $config['mail_order'];
        $body = str_replace('{name_pay}', $order->recipients_name, $body);
        $body = str_replace('{time}', $order->created_time, $body);
        $body = str_replace('{mail_pay}', $order->recipients_email, $body);
        $body = str_replace('{phone_pay}', $order->recipients_telephone, $body);
        $body = str_replace('{id_order}', 'DH' . str_pad($id, 8, "0", STR_PAD_LEFT), $body);
        $body = str_replace('{name_re}', $order->sender_name, $body);
        $body = str_replace('{mail_re}', $order->sender_email, $body);
        $body = str_replace('{add_re}', $order->sender_address . ', ' . $order->sender_wards . ', ' . $order->sender_district . ', ' . $order->sender_province, $body);
        $body = str_replace('{phone_re}', $order->sender_telephone, $body);
        if ($order->ord_payment_type == 1) {
            $payType = 'Chuyển khoản ngân hàng';
        } elseif ($order->ord_payment_type == 2) {
            $payType = 'Giao hàng - nhận tiền (COD)';
        } elseif ($order->ord_payment_type == 3) {
            $payType = 'Thanh toán qua cổng thanh toán online';
        }
        if ($order->fee) {
            $fee = format_money($order->fee);
        } else
            $fee = '0 đ';
//        var_dump($fee);
//        var_dump($order->total_before_discount);
        $body = str_replace('{pay_type}', $payType, $body);
        $body = str_replace('{fee}', $fee, $body);

        if ($order->total_after_discount && $order->fee) {
            $money = format_money($order->total_after_discount + (double)$order->fee);
        } elseif (!$order->total_after_discount && $order->fee)
            $money = format_money((double)$order->fee);
        elseif ($order->total_after_discount && !$order->fee)
            $money = format_money($order->total_after_discount);

        elseif (!$order->total_after_discount && !$order->fee)
            $money = "0 đ";
//        var_dump($order->total_after_discount);
//        var_dump($money);die;
        $body = str_replace('{fee}', $fee, $body);
        $body = str_replace('{total}', format_money($order->total_before_discount), $body);

        if ($order->discount_money) {
            $dis_mon = format_money($order->discount_money);
        } else
            $dis_mon = '0 đ';
...
//        print_r($bodyitem);die;

        $bodyitem2 = str_replace("{item2}", $bodyitem, $bodyitem2);
//        print_r($bodyitem2);die;

        $body = str_replace("<p>{item}</p>", $bodyitem2, $body);
//print_r($body);die;

        $rs = $this->send_email1("Geni - Xác nhận đơn hàng", $body, $order->recipients_name, $order->recipients_email, '', 1);
        if (!$rs)
            return false;
        return true;
