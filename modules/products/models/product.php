...
//        var_dump($where);
    ...
    function get_size_dau($id,$color_id)
    {
        global $db;
        $fs_table = FSFactory::getClass('fstable');
        $query = "SELECT * FROM " . $fs_table->getTable('fs_products_sub') . "
                                  WHERE 
                                     published = 1 and product_id = $id and color_id = $color_id 
                    ORDER BY id ASC 
                                 ";
        $db->query($query);
        $list = $db->getObject();
    ...
    function get_price_ajax($id,$color,$size)
    {
        global $db;
        $fs_table = FSFactory::getClass('fstable');
        $query = "SELECT * FROM " . $fs_table->getTable('fs_products_sub') . "
                                  WHERE 
                                     published = 1 and product_id = $id and color_id = $color and size_id = $size
...
//        echo $query;
    ...
    function get_color_dau($id,$size)
    {
        global $db;
        $fs_table = FSFactory::getClass('fstable');
        $query = "SELECT * FROM " . $fs_table->getTable('fs_products_sub') . "
                                  WHERE 
                                     published = 1 and product_id = $id and size_id = $size 
                    ORDER BY id ASC 
                                 ";
        $db->query($query);
        $list = $db->getObject();
...
//            var_dump($query);
    ...
    function list_shop($id_shop)
    {
        ...
        $query = " SELECT *
					FROM fs_products
					WHERE
						published = 1 and id_shop = $id_shop AND id !=  $id 
					 ORDER BY  id DESC
					 LIMIT 4
							";
        $db->query($query);
    ...
    function love($id,$user_id)
    {
        
        global $db;
        $query = " SELECT *
					FROM fs_products_favourite
					WHERE record_id = $id and user_id = $user_id ORDER BY  id DESC
							";
        $db->query($query);
        $list = $db->getObject();
        return $list;
