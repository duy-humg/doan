<?php
global $tmpl;
$tmpl->addStylesheet ( 'click', 'blocks/services_menu/assets/css' );
$tmpl->addScript ( 'services_menu_click', 'blocks/services_menu/assets/js' );
$Itemid = 5; // config
$num_child = array ();
$parant_close = 0;
$i = 0;
...
foreach ( $list as $item ) {
	...
	if ($item->level) {
		...
		if ($group_activated) {
			...
			$html_activate .= "<h2 class='h2_" . $item->level . "'><a href='" . $link . "' ><span> " . $item->name . "</span></a></h2>  ";
		} else {
